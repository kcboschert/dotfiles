---
name: update-pi-learnings
version: 2.0.0
description: |
  Propagate ONE validated learning about Pi-coding-agent behavior from this session into the project's
  AGENTS.md — the MAIN-AGENT memory Pi loads every session, not agent or skill files. Extract the
  ...
  make a Pi-coding-agent-behavior rule persist into future sessions.
allowed-tools:
  - AskUserQuestion
  - Read
  - Edit
...
arguments:
  - request
when_to_use: |
  Use when the user explicitly asks to record a Pi-coding-agent behavior or workflow learning in the project's
  AGENTS.md — \"/update-pi-learnings\", \"add this Pi-coding-agent behavior to AGENTS.md\", \"record this workflow
  ...
  durable Pi memory and is user-only.
source: https://github.com/ulpi-io/skills/tree/main/update-claude-learnings
---

<EXTREMELY-IMPORTANT>
This skill updates durable Pi memory and must stay disciplined.

Non-negotiable rules:

1. Only record learnings about Pi-coding-agent behavior or workflow in this project.
...
4. Get explicit user confirmation before modifying `AGENTS.md`.
...
Add one validated Pi-coding-agent-behavior learning to project memory by:

- extracting the right rule from the session
- verifying it belongs in `AGENTS.md``}},{set_line:{anchor:
...
This skill is only for persistent Pi-coding-agent behavior in this project.

Valid examples:

- workflow rules for when Pi should use a specific skill
- scope-control rules for how Pi should ask before expanding work
- session-management rules for checkpoints, progress updates, or stopping conditions
- project-specific Pi-coding-agent behavior that improves future sessions
...
If the learning does not belong in `AGENTS.md`, stop and say where it should go instead.

**Success criteria**: The learning clearly belongs in persistent Pi project memory.
...
## Step 2: Check the current AGENTS memory and choose placement

Read the project `AGENTS.md`. If it does not exist, use
`references/agents-md-template.md` as the structural fallback.
...
Before editing `AGENTS.md`, present:
...
- if `AGENTS.md` is missing, create it using the provided template and then add the learning
...
**Success criteria**: `AGENTS.md` contains the approved learning in the right place without collateral churn.
...
**Success criteria**: The user can see exactly what durable Pi memory changed.
...
- Do not rewrite large parts of `AGENTS.md` when a small targeted insertion is enough.

## When To Load References

- `references/learning-scope.md`
  Use for deciding whether the learning belongs in Pi memory, choosing section placement, handling duplicates, and checking common failure modes.

- `references/agents-md-template.md`
  Use only when `AGENTS.md` is missing or its structure needs a minimal compatible fallback.
...
4. whether `AGENTS.md` was created or updated
